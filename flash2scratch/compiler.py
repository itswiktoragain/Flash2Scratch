...
class AS3Compiler:
    ...
    def __init__(self, project, program, report, fps=30, frame_map=None):
        self.p = project
    ...
    def _backdrop_literal(self, text):
        value = self._literal(text)
    ...
    def _key_branches(body: str) -> list[tuple[str, str, tuple[int, int]]]:
        patterns = [
            ...
            re.compile(r"if\s*\(\s*Key\.isDown\s*\(\s*([^)]+)\s*\)\s*\)\s*\{([^{}]*)\}", re.S),
        ]
        found = []
    ...
    def _listener(self, listener, handler):
        ...
        if listener.event == "Event.ENTER_FRAME":
            ...
            for key, key_body, _ in key_branches:
                ...
                builder.blocks[hat]["next"] = self._body(builder, key_body, hat, target_name)
            remaining = self._remove_spans(handler.body, [span for _, _, span in key_branches])
            if remaining.strip():
        ...
        if listener.event == "MouseEvent.CLICK":
            ...
            builder.blocks[hat]["next"] = self._body(builder, handler.body, hat, target_name)
        ...
        if listener.event == "KeyboardEvent.KEY_DOWN":
            ...
            if found:
                ...
                self.report.unsupported.append(f"{listener.handler}: dynamic KEY_DOWN handler")
            return
    ...
    def _body(self, builder, body, parent, target_name="stage"):
        ...
        for statement in split_statements(body):
            block = self._stmt(builder, " ".join(statement.split()), parent, target_name)
            if block:
                ids.append(block)
        if ids:
            builder.chain(ids)
    ...
    def _canonical_property(prop: str) -> str:
        return {"_x":"x","_y":"y","_rotation":"rotation","_visible":"visible","_alpha":"alpha","_xscale":"scaleX","_yscale":"scaleY"}.get(prop, prop)

    def _stmt(self, builder, statement, parent, target_name="stage"):
        match = re.fullmatch(r"(?:(?:_root\.)?([A-Za-z_$][\w$]*)\.)?(x|y|rotation|_x|_y|_rotation)\s*([+\-])=\s*([-+]?\d+(?:\.\d+)?)", statement)
        if match:
            obj, prop, op, number = match.groups(); obj = obj or target_name; prop = self._canonical_property(prop); value = float(number) * (1 if op == "+" else -1)
            if obj == target_name and target_name != "stage":
                if prop == "x":
                    self.report.translated.append(statement); return builder.add("motion_changexby", parent=parent, inputs={"DX": builder.num(value)})
                if prop == "y":
                    self.report.translated.append(statement); return builder.add("motion_changeyby", parent=parent, inputs={"DY": builder.num(-value)})
                if prop == "rotation":
                    self.report.translated.append(statement); opcode = "motion_turnright" if value >= 0 else "motion_turnleft"; return builder.add(opcode, parent=parent, inputs={"DEGREES": builder.num(abs(value))})
            var_name = f"{obj}.{prop}"; vid = self.p.global_var(var_name); self.report.translated.append(statement)
            return builder.add("data_changevariableby", parent=parent, inputs={"VALUE": builder.num(value)}, fields={"VARIABLE": [var_name, vid]})

        match = re.fullmatch(r"(?:_root\.)?([A-Za-z_$][\w$]*)\s*([+\-])=\s*([-+]?\d+(?:\.\d+)?)", statement)
        if match:
            name, op, value = match.groups(); vid = self.p.global_var(name); delta = float(value) * (1 if op == "+" else -1); self.report.translated.append(statement)
            return builder.add("data_changevariableby", parent=parent, inputs={"VALUE": builder.num(delta)}, fields={"VARIABLE": [name, vid]})

        match = re.fullmatch(r"(?:_root\.)?([A-Za-z_$][\w$]*)\s*=\s*(.+)", statement)
        if match:
            name, value = match.groups(); vid = self.p.global_var(name); self.report.translated.append(statement)
            return builder.add("data_setvariableto", parent=parent, inputs={"VALUE": builder.text(self._literal(value))}, fields={"VARIABLE": [name, vid]})

        match = re.fullmatch(r"(?:(?:this|_root)\.)?gotoAnd(?:Stop|Play)\s*\(([^)]+)\)", statement)
        if match:
            self.report.translated.append(statement)
            return builder.add("looks_switchbackdropto", parent=parent, inputs={"BACKDROP": builder.text(self._backdrop_literal(match.group(1)))})

        if re.fullmatch(r"(?:(?:this|_root)\.)?nextFrame\s*\(\s*\)", statement):
            self.report.translated.append(statement); return builder.add("looks_nextbackdrop", parent=parent)

        match = re.fullmatch(r"(?:trace|console\.log)\s*\((.*)\)", statement)
        if match:
            self.report.translated.append(statement); return builder.add("looks_say", parent=parent, inputs={"MESSAGE": builder.text(self._literal(match.group(1)))})

        if statement and not re.match(r"^(var |const |import |package |super\s*\(|stop\s*\(|play\s*\()", statement):
